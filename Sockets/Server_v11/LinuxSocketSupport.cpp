...
bool winuxBindToSocket() {
	...
	// int option = 1;
	// result = setsockopt(ListeningSocket, SOL_SOCKET, (SO_REUSEPORT | SO_REUSEADDR), (char*)&option, sizeof(option));
	// if (result < 0) {
	// 	std::cout << "Could not set options.  setsockopt() returned error code " << result << std::endl;
	// 	return false;
	// }
